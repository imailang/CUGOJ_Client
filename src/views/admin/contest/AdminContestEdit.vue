<template>
  <el-card>
    <template #header>
      <el-page-header content="修改比赛" @back="goBack" />
    </template>
    <el-form
      ref="contestInfoRef"
      :model="contestInfo"
...
const updateContestProblems = () => {
  api.contest.getContestProblems(contestID.value).then((response) => {
...
const updateContest = () => {
  api.contest.getFullContest(contestID.value).then((response) => {
    if (typeof response === "undefined" || response.Statu !== "000") {
      ElMessage.error("请求出错");
      return;
    }
    var tmp = JSON.parse(response.Info);
    contestInfo.authur = tmp.Authur;
    contestInfo.title = tmp.Title;
    contestInfo.type = tmp.Type.toString();
    contestInfo.source = tmp.Source;
    contestInfo.owner = tmp.Owner.toString();
    contestInfo.visible = tmp.Visible.toString();
    contestInfo.password = tmp.Password;
    contestInfo.startTime = tmp.StartTime;
    contestInfo.endTime = tmp.EndTime;
    contestInfo.profile = turndownService.turndown(tmp.Profile);
    contestInfo.description = turndownService.turndown(tmp.Description);
    contestInfo.sealRank = tmp.SealRank;
    contestInfo.sealRankTime = tmp.SealRankTime;
    contestInfo.openPrint = tmp.OpenPrint;
    contestInfo.rankShowName = tmp.RankShowName;
    contestInfo.openOutRank = tmp.OpenOutRank;
    contestInfo.registeMode = tmp.RegisteMode.toString();
  });
};
